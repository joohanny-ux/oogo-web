...
export const ARCHIVE_CLIENT_MAX_EDGE = 2400;
...
/**
 * Downscale large archive images in the browser so they can reach the upload API on Vercel.
...
export async function prepareArchiveUploadFile(file: File): Promise<File> {
  ...
  try {
    ...
    const scale = Math.min(1, ARCHIVE_CLIENT_MAX_EDGE / longest);
    ...
    const baseName = file.name.replace(/\.[^.]+$/, "") || "archive-image";
